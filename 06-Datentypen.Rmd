...
`NULL` wird euch vermutlich weniger häufig begegnenals `NA`, zumindest in Datensätzen.  
Ansonsten taucht `NULL` eher bei R-Funktionen als *default argument* auf, also ein Argument einer Funktion, dass nicht gesetzt, außer ihr setzt es explizit. Das klingt jetzt etwas abstrakt, aber wir werden im Laufe dieser Einführung vermutlich noch Beispiele dafür sehen.
...
Habt ihr schonmal durch 0 geteilt? Oder den Tangens von 0 bestimmt?  
...
Später werden wir noch andere Klassen für Tabellen sehen, die `data.frame` erweitern bzw. etwas aufhübschen, namentlich werden das `tbl_df` oder eher `tibble` sein, aber dazu müssen wir uns erst *Packages* ansehen.  
