...
Und was ist mit den **Taus**?  
Es ist kompliziert.  
Zum einen sei gesagt, dass wir $\tau_b$ via `cor()` bekommen können:

```{r ordstats_3}
cor(qmsurvey$lernen, qmsurvey$zufrieden, method = "kendall")
```

Als nächstes sei gesagt, dass die anderen Taus eh selten benutzt werden.  
Aber na gut, wir können für den Rest auf das package `DescTools` zurückgreifen:

```{r ordstats_4}
library(DescTools)

StuartTauC(qmsurvey$lernen, qmsurvey$zufrieden)
KendallTauA(qmsurvey$lernen, qmsurvey$zufrieden)
```

…Vermutlich sollten wir die Taus auch noch alle zusammen in eine einzelne `tadaatoolbox`-Funktion stecken, das macht so ja keinen Spaß.  
