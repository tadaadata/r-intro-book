...
```{r, echo=FALSE, fig.cap="Output Optionen f&uuml;r diversen Kram", out.width="50%"}
...
Das oben ist der [*YAML*](https://en.wikipedia.org/wiki/YAML) header. YAML ist auch eine Markup-Sprache wie HTML, und weil es so viele dieser Markup-Sprachen gibt, heißt YAML lang "**Y**et **A**nother **M**arkup **L**anguage"[^yaml]. Es gibt auch noch *HAML* und *TOML*.  
...
[^yaml]: Alternativ kann YAML auch für ***Y**AML **A**in't **M**arkup **L**anguage* stehen, wenn ihr [rekursive Akronyme](https://de.wikipedia.org/wiki/Rekursives_Akronym) bevorzugt
