...
ggplot(data = gotdeaths_perseason, aes(x = death_season, y = deaths)) +
  ...
  labs(
    title = "Game of Thrones", subtitle = "Deaths per Season",
    x = "Season", y = "Deaths"
  )
```
...
```{r regression-pretty-tab, eval=FALSE}
# library(sjPlot)
tab_model(model)
```
...
```{r regression-augment, warning=FALSE, message=FALSE}
augment(model) %>%
